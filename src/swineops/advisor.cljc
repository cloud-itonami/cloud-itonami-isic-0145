(ns swineops.advisor
  "SwineOpsAdvisor -- the contained LLM/decision node. This actor's
  intelligence layer proposes back-office coordination actions (herd
  record logging including farrowing data, veterinary scheduling,
  health/biosecurity-concern flags, supply procurement) based on facility
  state and operator input. The advisor is SEALED into the `:advise` step
  of the operation flow; every proposal is routed through the independent
      ...
      (case op
        ...
        {:op :log-herd-record
         ...
         :value {:facility-id facility-id
                 ...
                 :farrowing-count (:farrowing-count request nil)}
         :cites ["operator-submitted-count"]
         :summary "Herd count/weight/health-check/farrowing entry logged from operator submission"
         :confidence 0.9}
        ...
        {:op :flag-animal-health-concern
         ...
         :summary "Animal health/biosecurity concern (e.g. suspected ASF) flagged for veterinary/farm-operator review"
         :confidence 0.8}
