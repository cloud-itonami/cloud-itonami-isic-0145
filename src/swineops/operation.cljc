(ns swineops.operation
  "OperationActor -- one swine-farm operation = one supervised actor run.
  The advisor (SwineOpsAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Swine Farm Operations
  ...
  One run = one swine-farm coordination operation (intake -> advise ->
  ...
  Governor is clean -- see `swineops.governor/always-escalate-ops`.
  ...
  (:require [swineops.advisor :as advisor]
...
(defn run-operation
  "Run one swine-farm operation through the advisor -> governor -> phase
...
(defn build
  ...
  run-operation for testing. opts:
    :advisor -- a `swineops.advisor/Advisor` (default: mock-advisor)"
  [store & [opts]]
