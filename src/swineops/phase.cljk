(ns swineops.phase
  "Phase gate for the swine-farm operations coordinator rollout (0->3
  maturity). Mirrors `cattleops.phase` (cloud-itonami-isic-0141) in
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`swineops.governor/always-escalate-ops`) escalate even when clean;
  ...
  (:require [swineops.governor :as governor]))
