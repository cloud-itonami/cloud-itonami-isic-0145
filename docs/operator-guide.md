# Operator Guide: Swine-Farm Operations Coordinator

## Overview

The Swine-Farm Operations Coordinator is a facility-management robot that:

1. **Logs operational data** — herd counts, weights, health notes, farrowing data
2. **Schedules coordination** — veterinary appointments, supply orders
3. **Escalates concerns** — any animal health or biosecurity issue (e.g. suspected ASF)
...
1. **Facility Registration** — your barn/pen facility must be registered in
...
3. **Clear Request Type** — specify what you're doing:
   - `:log-herd-record` — record herd data (including farrowing)
...
2. **Actor Processes** (`operation/run-operation store request context`)
   - `:advise` — `SwineOpsAdvisor` proposes an action (`swineops.advisor`)
   - `:govern` — `SwineFarmOperationsGovernor` checks hard invariants and escalation gates (`swineops.governor`)
   - phase gate — rollout-phase constraints applied on top of the Governor's verdict (`swineops.phase`)
...
- `:flag-animal-health-concern` — any welfare or biosecurity issue (e.g. suspected ASF)
...
`swineops.operation` is currently a synchronous stub (see its docstring):
...
The actor provides a standard protocol (`swineops.store/Store`) for backend
integration:

- **Facility lookup** — `(store/registered-facility store facility-id)`

Implementations include in-memory `MemStore` (testing, `swineops.store`),
